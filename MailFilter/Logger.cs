...
namespace MailFilter
{
    ...
    public class Logger : IDisposable
    {
        ...
        {
            WriteInternal(LogSeverity.Debug, text, arguments).ToList();
        }

        public void Verbose(string text, params object[] arguments)
        {
            WriteInternal(LogSeverity.Verbose, text, arguments).ToList();
        }

        public void Info(string text, params object[] arguments)
        {
            WriteInternal(LogSeverity.Info, text, arguments).ToList();
        }

        public void Notice(string text, params object[] arguments)
        {
            WriteInternal(LogSeverity.Notice, text, arguments).ToList();
        }

        public void Warning(string text, params object[] arguments)
        {
            WriteInternal(LogSeverity.Warning, text, arguments).ToList();
        }

        public void Error(string text, params object[] arguments)
        {
            WriteInternal(LogSeverity.Error, text, arguments).ToList();
        }

        public Exception Critical(string text, params object[] arguments)
        {
            var entry = WriteInternal(LogSeverity.Critical, text, arguments).ToList().First();
            return new Exception(entry.Text);
        }

        public void Write(LogSeverity severity, string text, params object[] arguments)
        {
            WriteInternal(severity, text, arguments).ToList();
        }

        private IEnumerable<LogEntry> WriteInternal(LogSeverity severity, string text, params object[] arguments)
        {
